...
cuts['bkg']='('+cuts['resW']+'||'+cuts['nonRes']+')'
...
bkgTemplate='ntuples2016/TT_pow,ntuples2017/TTHad_pow,ntuples2017/TTLep_pow,ntuples2017/TTSemi_pow,ntuples2018/TTHad_pow,ntuples2018/TTLep_pow,ntuples2018/TTSemi_pow,ntuples2016/WJetsToLNu_HT,ntuples2016/WWToLNuQQ,ntuples2016/WZTo1L1Nu2Q,ntuples2017/WJetsToLNu_HT,ntuples2017/WWToLNuQQ,ntuples2018/WJetsToLNu_HT,ntuples2018/WWToLNuQQ,ntuples2016/DYJetsToLL_M50_HT,ntuples2017/DYJetsToLL_M50_HT,ntuples2017/T_tW,ntuples2017/TBar_tW,ntuples2018/DYJetsToLL_M50_HT'

XWWTemplate='ntuples2016/VBF_RadionToWW_narrow,ntuples2017/VBF_RadionToWW_narrow,ntuples2018/VBF_RadionToWW_narrow'
XWZTemplate='ntuples2017/VBF_WprimeToWZ_narrow,ntuples2018/VBF_WprimeToWZ_narrow'


# Settings
jets=['control','W','Z','combined']
leptons=['e','mu','lep']
purities=['HP','LP','allP']
categories=['nob','b']
...
cuts['acceptanceGENMVV']="(lnujj_gen_partialMass>{minMVV}&&lnujj_gen_partialMass<{maxMVV})".format(minMVV=800,maxMVV=5000)
    ...
    for lep in leptons:
      for pur in purities:
        for cat in categories:
          cut='*'.join([cuts['common'],cuts[lep],cuts[pur],cuts[jet],cuts[cat],cuts['dijet'],cuts['eta'],cuts['deta'],cuts['mjj'],cuts['SDMass'],cuts['resMass']])
          rootFile=filename+'_MVV_'+lep+'_'+pur+'_'+jet+'_'+cat+'.root'
          ...
          os.system(cmd)
          jsonFile=filename+'_MVV_'+lep+'_'+pur+'_'+jet+'_'+cat+'.json'
          print 'Making JSON'
  ...
  for jet in jets:
    for lep in leptons:
      for pur in purities:
        for cat in categories:
          cut='*'.join([cuts['common'],cuts[lep],cuts[pur],cuts[jet],cuts[cat],cuts['dijet'],cuts['eta'],cuts['deta'],cuts['mjj'],cuts['SDMass'],cuts['resMass']])
          yieldFile=filename+"_"+lep+"_"+pur+"_"+jet+"_"+cat+"_yield"
...
def makeBackgroundShapesMVV(name,filename,template,addCut='1'):
  ...
  for jet in jets:
    for lep in leptons:
      for pur in purities:
        for cat in categories:
          cut='*'.join([cuts['common'],cuts[lep],cuts[pur],cuts[cat],cuts['dijet'],cuts[jet],cuts['eta'],cuts['deta'],cuts['mjj'],cuts['acceptanceGENMVV'],addCut])
          rootFile=filename+"_"+name+"_MVV_"+lep+"_"+pur+"_"+jet+"_"+cat+".root"
...
def makeNormalizations(name,filename,template,data=0,addCut='1',factor=1):
  for jet in jets:
    for lep in leptons:
      for pur in purities:
        for cat in categories:
          cut='*'.join([cuts['common'],cuts[lep],cuts[pur],cuts[jet],cuts[cat],cuts['dijet'],cuts['eta'],cuts['deta'],cuts['mjj'],cuts['SDMass'],cuts['resMass'],addCut])
          rootFile=filename+"_"+lep+"_"+pur+"_"+jet+"_"+cat+".root"
...
# Signal templates
if DOSIGNALSHAPES:
  if DOXWW: makeSignalShapesMVV(inputDir+'LNuJJ_XWW',XWWTemplate)
  if DOXWZ: makeSignalShapesMVV(inputDir+'LNuJJ_XWZ',XWZTemplate)

if DOSIGNALYIELDS:
  if DOXWW: makeSignalYields(inputDir+"LNuJJ_XWW",XWWTemplate,BRVBFWW,{'HP':1.03,'LP':0.95})
  if DOXWZ: makeSignalYields(inputDir+"LNuJJ_XWZ",XWZTemplate,BRVBFWZ,{'HP':1.03,'LP':0.95})

# Background templates
if DORESONANT: makeBackgroundShapesMVV("resW",inputDir+"LNuJJ",resWTemplate,cuts['resW'])
if DONONRESONANT: makeBackgroundShapesMVV("nonRes",inputDir+"LNuJJ",nonResTemplate,cuts['nonRes'])

if DONORMMC:
  makeNormalizations("resW",inputDir+"LNuJJ",resWTemplate,0,cuts['resW'],1.0)
  makeNormalizations("nonRes",inputDir+"LNuJJ",nonResTemplate,0,cuts['nonRes'],1.0)

if DONORMDATA: makeNormalizations("data",inputDir+"LNuJJ",dataTemplate,1)
