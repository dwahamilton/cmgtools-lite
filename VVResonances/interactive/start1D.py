...
def getPlotters(samples,folder,isData=False,corr="1"):
    ...
    plotters=[]
    for filename in os.listdir('ntuples/'+folder):
        for sampleType in sampleTypes:
            if filename.find(sampleType)!=-1:
                ...
                plotters.append(TreePlotter('ntuples/'+folder+'/'+fname+'.root','tree'))
                if not isData:
                    plotters[-1].setupFromFile('ntuples/'+folder+'/'+fname+'.pck')
...
for jet in jets:
  for cat in categories:
    BKGDataStack2016[jet,cat]=StackPlotter()
    BKGDataStack2016[jet,cat].addPlotter(VJets2016,'WJets','W+Jets','background')
    BKGDataStack2016[jet,cat].addPlotter(WVT2016,'WVT','W+V/t','background')

    BKGDataStack2017[jet,cat]=StackPlotter()
    BKGDataStack2017[jet,cat].addPlotter(VJets2017,'WJets','W+Jets','background')
    BKGDataStack2017[jet,cat].addPlotter(WVT2017,'WVT','W+V/t','background')

    BKGDataStack2018[jet,cat]=StackPlotter()
    BKGDataStack2018[jet,cat].addPlotter(VJets2018,'WJets','W+Jets','background')
    BKGDataStack2018[jet,cat].addPlotter(WVT2018,'WVT','W+V/t','background')

    BKGDataStack[jet,cat]=StackPlotter()
    BKGDataStack[jet,cat].addPlotter(VJets,'WJets','W+Jets','background')
    BKGDataStack[jet,cat].addPlotter(WVT,'WVT','W+V/t','background')

    if jet=='control' or cat=='b':
      BKGDataStack2016[jet,cat].addPlotter(data2016,'data','data','data')
      BKGDataStack2017[jet,cat].addPlotter(data2017,'data','data','data')
      BKGDataStack2018[jet,cat].addPlotter(data2018,'data','data','data')
      BKGDataStack[jet,cat].addPlotter(data,'data','data','data')
...
f=ROOT.TFile("fig/1DHist.root","RECREATE")
f.cd()
  ...
  for sample in ['bkgDataStack','bkg','resW','nonRes','radion']:
    for lep in leptons:
      for pur in purities:
        for jet in jets:
          for cat in categories:
            f.mkdir('/'.join([year,sample,lep+pur+jet+cat]))
            if sample=='radion':
              for mass in masses[year]:
                f.mkdir('/'.join([year,sample,lep+pur+jet+cat,mass]))
...
for key,sample in BKGStackDict.items():
  ...
  for lep in leptons:
    for pur in purities:
      for jet in jets:
        for cat in categories:
          f.cd('/'.join([year,folder,lep+pur+jet+cat]))
          lepCut='*'.join([cuts['common'],cuts[lep],cuts[pur],cuts[jet],cuts[cat],cuts['dijet'],cuts['eta'],cuts['deta'],cuts['mjj'],cuts['SDMass'],cuts['resMass']])
          stack=sample[jet,cat].drawStack('lnujj_LV_mass',lepCut,lumi,bins['sigMass'][0],bins['sigMass'][1],bins['sigMass'][2],'m_{WV}','GeV')
  ...
  for lep in leptons:
    for pur in purities:
      for jet in jets:
        for cat in categories:
          for mass in sample:
            f.cd('/'.join([year,folder,lep+pur+jet+cat,mass]))
            lepCut='*'.join([cuts['common'],cuts[lep],cuts[pur],cuts[jet],cuts[cat],cuts['dijet'],cuts['eta'],cuts['deta'],cuts['mjj'],cuts['SDMass'],cuts['resMass']])
